...
async function renderContent(title) {
    ...
    if (version && edit) {
        ...
        for (var i=0; i<range.values.length; i++) {
            document.querySelector('#version-list>tbody').innerHTML = '<tr><td>v'+(range.values.length-1-i)+'</td><td>'+range.values[(range.values.length-1-i)][1]+'</td><td><a href="./?d=title&v='+i+'">읽기</a> <a href="./?d=title&e='+i+'">이 버전으로부터 편집</a></td></tr>';
        }
    } else if (version) {
        const output = range.values[version][2]
